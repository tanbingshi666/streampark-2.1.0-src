...
object KafkaSource {
  ...
  def getSource[T: TypeInformation](
    ...
    // 过滤 application.yaml 配置前缀 kafka.source. 的信息
    val prop = ConfigUtils.getConf(
      // 全局配置参数
      ctx.parameter.toMap,
      // 默认 kafka.source.
      KAFKA_SOURCE_PREFIX + alias)
    // 拷贝自定义的 kafka Property 到 prop
    ...
    val consumer = (topicOpt, regexOpt) match {
      ...
      case (Some(top), _) =>
        ...
        // 创建 Flink 消费 kafka 标配 source FlinkKafkaConsumer
    ...
    // kafka consumer 是否自动提交 offset 默认 true
