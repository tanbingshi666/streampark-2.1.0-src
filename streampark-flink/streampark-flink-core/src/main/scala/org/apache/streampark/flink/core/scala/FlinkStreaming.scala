...
/**
 * StreamPark 封装了 Flink DataStream 相关的执行环境
 * 比如 ParameterTool(全局配置参数)、StreamExecutionEnvironment (Flink DataStream 标配执行环境)
 *
 * @param parameter
 * @param environment
 */
class StreamingContext(
                        // 全局配置参数 (包括系统环境变量参数、application.yaml 定义参数、程序入口参数)
                        val parameter: ParameterTool,
                        // Flink DataStream 标配执行环境 StreamExecutionEnvironment
  ...
  /**
   * for Java
   * 此处一般使用 StreamPark 提供的 Java API 运行 Flink DataStream Job 入口
   */
