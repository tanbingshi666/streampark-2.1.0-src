...
public class FlinkEnvServiceImpl extends ServiceImpl<FlinkEnvMapper, FlinkEnv>
    ...
    public Integer check(FlinkEnv version) {
         ...
         * 一般在界面填写如下内容：
         * {
         * flinkName: flink-1.16.0,
         * flinkHome: /opt/app/flink/flink-1.16.0,
         * teamId: 10000
         * }
         */
        ...
        // 1) check name
        /**
         * 根据 flink_name 查询 t_flink_env 表
         * 判断是否名字重复
         */
        ...
        // 2) check dist_jar
        /**
         * 判断 ${FLINK_HOME}/lib 是否存在 flink-dist*.jar
         */
    ...
    public boolean create(FlinkEnv version) throws Exception {
        ...
        // 默认创建第一个 flink 环境变量为默认
        version.setIsDefault(count == 0);
        version.setCreateTime(new Date());
        // 读取 ${FLINK_HOME}/conf/flink-conf.yaml 文件内容并压缩成字符串
        version.doSetFlinkConf();
        // 查询 flink 以及 scala 版本
        version.doSetVersion();
        // 往 t_flink_env 表插入一条记录
        return save(version);
    }
