...
public class ProjectServiceImpl extends ServiceImpl<ProjectMapper, Project>
    ...
    public void build(Long id) throws Exception {
        // 查询 t_flink_project 表
        Project project = getById(id);
        // 更新 build project 状态
        this.baseMapper.updateBuildState(project.getId(), BuildState.BUILDING.get());
        // 获取 build log 路径
        // 格式: ${WORKSPACE}/logs/build_logs/${project_id}/build.log
        // 比如: /opt/app/streampark/streampark-2.1.0/workspace/logs/build_logs/100002/build.log
        ...
        ProjectBuildTask projectBuildTask =
            new ProjectBuildTask(
                ...
                fileLogger -> {
                    // 根据 project id 查询 t_flink_app 表获取该 project 涉及的 application
                    List<Application> applications =
                        this.applicationService.getByProjectId(project.getId());
                    // 存在 则更新对应 application 的状态
                    applications.forEach(
                        (app) -> {
                            fileLogger.info(
