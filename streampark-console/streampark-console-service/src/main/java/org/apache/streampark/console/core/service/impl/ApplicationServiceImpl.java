...
public class ApplicationServiceImpl extends ServiceImpl<ApplicationMapper, Application>
    ...
    public boolean checkEnv(Application appParam) throws ApplicationException {
        // 根据 application id 查询 t_flink_app 表
        Application application = getById(appParam.getId());
        try {
            FlinkEnv flinkEnv;
            // 根据 flink version id 查询 t_flink_env 表
            ...
            // 检查 flink 环境 同时也会将 ${FLINK_HOME} 目录下的所有内容拷贝到指定目录 (HDFS LFS)
            envInitializer.checkFlinkEnv(
                /**
                 * 除了 YARN_APPLICATION 类型 -> StorageType.HDFS
                 * 其他都是 StorageType.LFS
                 */
                application.getStorageType(),
                flinkEnv);

            // 初始化存储对象(HdfsOperator or LfsOperator) 同时也会初始化目录以及上传指定的文件和jar
    ...
    public boolean checkBuildAndUpdate(Application application) {
        ...
        if (!build) {
            LambdaUpdateWrapper<Application> updateWrapper = Wrappers.lambdaUpdate();
            updateWrapper.eq(Application::getId, application.getId());
            // 如果在执行 build 之前 application 已经在运行 则更新 release 状态
            if (application.isRunning()) {
            ...
            // 如果是 flink sql job
            if (application.isFlinkSqlJob()) {
                // 查询 t_flink_sql 表获取 application 对应的 sql
                FlinkSql newFlinkSql = flinkSqlService.getCandidate(application.getId(), CandidateType.NEW);
    ...
    public void start(Application appParam, boolean auto) throws Exception {
        ...
        // 更新 application 状态为 starting
        starting(application);
        application.setAllowNonRestored(appParam.getAllowNonRestored());

        // 用户自定义 jar 的 main-class 配置
        // 也即 {$internal.application.main: custom_main_class}
        String appConf;
        // 用户自定义 jar 路径
        ...
        if (application.isCustomCodeJob()) {
            ...
            // 1) dist_userJar
            // 获取 streampark-flink-sql-client jar
            ...
            if (ExecutionMode.YARN_APPLICATION.equals(executionMode)) {
                ...
                // flinkUserJar 等于 HDFS 路径下的 /client/streampark-flink-sqlclient_2.12-2.1.0.jar
