...
public class AppBuildPipeServiceImpl
    ...
    public boolean buildApplication(@Nonnull Application app, ApplicationLog applicationLog) {
        ...
        // clear history
        // 删除之前 build pipe 也即删除表 t_app_build_pipe 对应的记录
        removeApp(app.getId());
                ...
                public void onStart(PipeSnapshot snapshot) {
                    ...
                    if (app.isCustomCodeJob()) {
                        ...
                        if (app.isUploadJob()) {
                            ...
                            // 将本地 cicd 之后的 jar (/dist/${project_id}/${module_name} 下的 jar)
                            // 上传到 HDFS /workspace/${application_id} 目录下
        ...
        // 异步 release pipeline application
        ...
        switch (executionMode) {
            case YARN_APPLICATION:
                ...
                if (app.getDevelopmentMode().equals(DevelopmentMode.CUSTOM_CODE)
                    ...
                    // HDFS -> ${workspace}/workspace/${application_id}
                    yarnProvidedPath = app.getAppHome();
                    // local -> ${workspace}/workspace/${application_id}
                    localWorkspace = app.getLocalAppHome();
                }
                // 创建 FlinkYarnApplicationBuildRequest
    ...
    private String retrieveFlinkUserJar(FlinkEnv flinkEnv, Application app) {
        switch (app.getDevelopmentMode()) {
            ...
            case FLINK_SQL:
                // 获取 /opt/app/streampark/streampark-2.1.0/client 目录下的 streampark-flink-sqlclient_2.12-2.1.0.jar
